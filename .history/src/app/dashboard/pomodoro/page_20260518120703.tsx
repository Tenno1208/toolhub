...
import { ArrowLeft, BrainCircuit, Play, Pause, RotateCcw, Coffee, Sparkles, Volume2, VolumeX, SunCozy, CloudRain } from 'lucide-react';
...
export default function PomodoroTool() {
  ...
  // Batas konvers waktu tiap mode sesi
  ...
  useEffect(() => {
    ...
    if (isActive && seconds > 0) {
      ...
      setIsActive(false);
      playSessionBell(); // Bunyikan alarm lokal bel renyah le
    }
  ...
  // 🚀 INOVASI BELL ALARM SYNTHESIS (Web Audio API Lokal)
  ...
  // 🚀 INOVASI AUDIO BACKGROUND GENERATOR (Murni Synthesizer Client-Side)
  const startAmbientSynth = () => {
    try {
      ...
      if (ambientType === 'rain') {
        osc.type = 'sawtooth';
        osc.frequency.setValueAtTime(45, ctx.currentTime); // Frekuensi deru rendah
      } else { // Waves ocean
        osc.type = 'sine';
        osc.frequency.setValueAtTime(60, ctx.currentTime);
        // Modulasi ayunan ombak naik turun meliuk halus
        osc.frequency.linearRampToValueAtTime(50, ctx.currentTime + 2);
      }

      gain.gain.setValueAtTime(0.04, ctx.currentTime); // Set volume tipis biar lo-fi rileks
      osc.connect(gain);
      gain.connect(ctx.destination);
    ...
    <div className="min-h-screen bg-slate-950 pt-24 pb-12 px-4 sm:px-8 text-slate-200 select-none">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-slate-900/40 border border-white/[0.05] backdrop-blur-md rounded-[32px] p-6 sm:p-8 text-center space-y-6 shadow-2xl relative overflow-hidden">
          ...
          {/* 🚀 INOVASI 1: PANEL NAVIGASI SAKELAR MULTI-MODE */}
          ...
          {/* 🚀 INOVASI 3: CHRONO DIGITAL TIMER MONITOR BAR PROGRESS */}
          ...
          {/* Tombol Utama Kontrol Kendali Jalannya Waktu */}
          ...
          {/* 🚀 INOVASI 2: BAR PANEL PEMUTAR SUARA AMBIENT AUDIO LATAR */}
          <div className="pt-4 border-t border-white/5 max-w-md mx-auto space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                { id: 'waves', label: '🌊 Ombak', icon: <SunCozy className="w-3.5 h-3.5" /> }
              ].map(item => (
                <button
