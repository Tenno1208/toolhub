...
import { ArrowLeft, Briefcase, Download, Eye, Code, Sparkles, Plus, Trash2, Laptop } from 'lucide-react';
...
export default function PortfolioBuilder() {
  ...
  // 🛠️ LOGIK GENERATOR HTML TEMPLATE (Premium Minimalist Dark Mode)
  const generateHTML = () => {
...
<body class="min-h-screen flex flex-col justify-between selection:bg-purple-500/30 selection:text-white">
    ...
    <main class="max-w-3xl mx-auto px-6 pt-20 pb-16 w-full flex-grow">
        ...
        <header class="mb-14">
            ...
            <p class="text-slate-400 leading-relaxed text-sm sm:text-base">${about || 'Tulis cerita singkat, visi, atau deskripsi profesional diri Anda di sini untuk menarik perhatian klien atau perekrut.'}</p>
            
            <div class="flex gap-5 mt-6">
                ${github ? `<a href="${github}" target="_blank" style="text-decoration: none;" class="text-xs font-bold text-slate-400 hover:text-white transition-colors flex items-center gap-1">💻 GitHub &rarr;</a>` : ''}
                ${linkedin ? `<a href="${linkedin}" target="_blank" style="text-decoration: none;" class="text-xs font-bold text-slate-400 hover:text-white transition-colors flex items-center gap-1">🔗 LinkedIn &rarr;</a>` : ''}
            </div>
        </header>
        ...
        <section class="mb-14">
            <h2 class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-4">Core Tech Stack</h2>
            <div class="flex flex-wrap gap-2">
                ...
                `).join('') : `
                    <span class="px-3.5 py-1.5 text-xs font-medium rounded-xl bg-white/[0.02] border border-white/[0.04] text-slate-500">HTML5</span>
                    <span class="px-3.5 py-1.5 text-xs font-medium rounded-xl bg-white/[0.02] border border-white/[0.04] text-slate-500">CSS3</span>
                    <span class="px-3.5 py-1.5 text-xs font-medium rounded-xl bg-white/[0.02] border border-white/[0.04] text-slate-500">JavaScript</span>
                `}
            </div>
        </section>

        <section class="mb-12">
            <h2 class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-6">Featured Projects</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ${projects.map(proj => proj.title ? `
                    <div class="p-5 rounded-2xl border border-white/[0.03] bg-white/[0.01] hover:border-purple-500/20 transition-all duration-300">
                        ...
                        <p class="text-xs text-slate-400 mb-4 leading-relaxed">${proj.description}</p>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-white">Instant Portfolio Builder v2</h1>
          </div>
        </div>
